...
static int visi=0, visi10min=64000, visi10mintag=0;
static long pres;
...
static int stat_t0=1, stat_u=1, stat_p=1, stat_wd=1, stat_ws=1, stat_rat=1, stat_raw=1, stat_vi=1;
...
static void* njdpthreadbelfort(void *t)
    {
    int i=0, j;
...
//	fd_ttyS1 = open(UART1, O_RDWR);
	...
	pthread_create(&th_njd, NULL, njdpthreadbelfort, 0);
...
//				printf("suiqy=%d,ludtemp=%d\n",suiqy,ludtemp);
		...
		if(tqxx_js==13 && xsyl==0)					// Added by XGR at 2017-12-29
			tqxx_js = 12;
		fzsj[TAG+37] = RI + '0';
...
//			printf("data=%x%x,zbtemp=%d\n",data[1],data[0],zbtemp);
            ...
            arg.pin_idx = PIO_INP_IDX0;		// PIO_LVL_INP1;
            ...
            close(fdfenzh);
 			...
 			if((fdfenzh=open("/usr/sensorstat.tmp",O_WRONLY|O_CREAT)) < 1)
                {
                goto whileend;
                }
            write(fdfenzh, stat_sensor, sizeof(stat_sensor));
            close(fdfenzh);
/*
